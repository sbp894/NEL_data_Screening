% function VJplot_screening_summary(allChinID)
all_allChinID=[241 272 269 299 315 316 313 314 311 312 259 266 268 302 212 251 291 286 230 280];
all_allChinID=reshape(all_allChinID, 5, 4)';

count=1;
for allAllVar=1:size(all_allChinID, 1)
    figure(1);
    clf;
    set(gcf, 'units', 'normalized', 'position', [.1 .1 .8 .8]);
    allChinID=all_allChinID(allAllVar, :);
    ...
    figStr=['/media/parida/DATAPART1/temp/QC_Figures/nh_tts_data_' num2str(count)];
    count=count+1;
    saveas(gcf, figStr);
    saveas(gcf, figStr, 'tiff');
end