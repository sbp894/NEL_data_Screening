function max_dBSPL=CalibInterp(cal_freq_kHz_all,calib)
% FILE: CalibInterp.m
...
max_dBSPL= nan(size(cal_freq_kHz_all));

for freqVar= 1:length(max_dBSPL)
    cal_freq_kHz= cal_freq_kHz_all(freqVar);
    ...
    if(calib(i,1)<cal_freq_kHz)
        ...
        max_dBSPL(freqVar)=(cal_freq_kHz-calib(i,1))/(calib(i+1,1)-calib(i,1))* ...
            (calib(i+1,2)-calib(i,2)) + calib(i,2);
    elseif(calib(i,1)>cal_freq_kHz)
        max_dBSPL(freqVar)=(cal_freq_kHz-calib(i-1,1))/(calib(i,1)-calib(i-1,1))* ...
            (calib(i,2)-calib(i-1,2)) + calib(i-1,2);
    elseif(calib(i,1)==cal_freq_kHz)
        max_dBSPL(freqVar)=calib(i,2);
    end
