function GENsave_UnitINFO(TrackNum,UnitNum,skipVERIFYdir)
% File: GENsave_UnitINFO(TrackNum,UnitNum,skipVERIFYdir)
% From: STMPsave_UnitINFO(TrackNum,UnitNum,skipVERIFYdir)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global CALIBpics ExpList

unitINFO.Info.CalibPIC_ToUse=CALIBpics(strcmp(ExpList,unitINFO.Info.ExpName));
picList=findPics('calib');
if isempty(find(picList==unitINFO.Info.CalibPIC_ToUse, 1))
	error('CALIB FILE NOT FOUND - check global CALIBpics')
end	
...
TrackNum=str2double(unitINFO.Info.Unit(1));
UnitNum=str2double(unitINFO.Info.Unit(3:4));

picList=findPics('tc',[TrackNum UnitNum]);

% Pick BF by hand 
[Thresh_dBSPL_ret,BF_kHz_ret,Q10_ret,TCdata] = plotTCs_BFbyhand(picList,unitINFO.Info.CalibPIC_ToUse,1,unitINFO.Info.SR_sps);
% beep
...
for ind=1:length(picList)
   ...
   unitINFO.TC{ind}.TCdata=TCdata{ind};	
end         

% If multiple TCs found, ask which to use
if length(picList)>1
   TCpicToUse=-999;
   while isempty(find(picList==TCpicToUse, 1))
...
for PICind=PIClist
   if isfield(DataList,'picBADlines')   % Use Stored values - if available
      unitINFO.PICS.picBADlines{PIClist==PICind} = DataList.picBADlines{PICind};
   else
      if ~sum(strcmp(getTAG(getFileName(PICind)),{'calib','tc'}))  % only process RASTER data files

         x=loadPic(PICind);
         fprintf('Picture #: %d, filename: %s',PICind,getfileName(PICind));
         ...
         unitINFO.PICS.picBADlines{PIClist==PICind} = excludeLines;
...
%
% 1/23/09 ADDED SR template - use this if available!
...
SRpicList=findPics('SR',[TrackNum,UnitNum]);

if ~isempty(SRpicList)
	SR_PICinds=find(ismember(SRpics,SRpicList));
else
	SR_PICinds=1:length(SRpics);
end

unitINFO.SRdata.SR_sps=mean(SRests_sps(SR_PICinds));
unitINFO.SRdata.SRpics=SRpics(SR_PICinds);
unitINFO.SRdata.SRests=SRests_sps(SR_PICinds);
unitINFO.SRdata.SRdata=SRdata(SR_PICinds);

if ~isempty(SRpicList)
	unitINFO.SRdata.allPICS.SR_sps=mean(SRests_sps);
	unitINFO.SRdata.allPICS.SRpics=SRpics;
	unitINFO.SRdata.allPICS.SRests=SRests_sps;
	unitINFO.SRdata.allPICS.SRdata=SRdata;
end
